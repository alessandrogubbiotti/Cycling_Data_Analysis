...
class TrainingPlotter:
    ...
    def plot_basic_training(self, data, save_path=None):
        """Plot basic training data without intervals - EXACT copy from your working script"""
        df = data['time_series']
    ...
    def plot_with_intervals(self, data, interval_type, save_path=None):
        """Plot training data with intervals highlighted - EXACT copy from your working script"""
        df = data['time_series']
        metadata = data.get('metadata', {})
        ...
        for interval in intervals:
            metrics = self.calculate_interval_metrics(interval, df, total_duration)
            if metrics:
        ...
        # Define colors for different interval types - EXACTLY as in your working script
        interval_colors = {
        ...
        # Highlight intervals and mark HR drift - EXACTLY as in your working script
        legend_added = set()
        for i, (interval, metrics) in enumerate(zip(intervals, all_metrics)):
            ...
            # Mark HR drift with text annotation on HR plot - WITH FANCY SYMBOLS
        ...
        # Create comprehensive metrics table with HR drift - EXACTLY as in your working script
        if all_metrics:
    ...
    def calculate_interval_metrics(self, interval, df, total_duration):
        """Calculate comprehensive interval metrics with HR drift analysis - EXACT copy"""
    ...
    def calculate_hr_drift(self, interval_data, interval):
        """Calculate detailed heart rate drift metrics - EXACT copy"""
        hr_metrics = {}
    ...
    def create_metrics_table(self, all_metrics, interval_source, save_path=None):
        """Create a comprehensive metrics table with HR drift analysis - EXACT copy"""
        ...
        # HR analysis table - WITH FANCY SYMBOLS AND COLOR CODING
        ...
        # Color cells based on HR drift - EXACTLY as in your working script
    ...
    def add_ramp_target_power(self, df, intervals):
        """Add linear increasing/decreasing target power for ramps, warmups, and cooldowns"""
        df_enhanced = df.copy()
        
        for interval in intervals:
            if interval.get('type') in ['ramp', 'warmup', 'cooldown']:
                start_time = self.safe_float_convert(interval.get('start_time', 0))
                end_time = self.safe_float_convert(interval.get('end_time', 0))
                power_low = self.safe_float_convert(interval.get('power_low', 0))
                power_high = self.safe_float_convert(interval.get('power_high', 0))
                ...
                if len(interval_points) > 0:
                    # Create linear progression from power_low to power_high (or vice versa)
                    timestamps = interval_points['timestamp'].values
                    progress = (timestamps - start_time) / (end_time - start_time)
                    
                    if interval.get('type') == 'cooldown':
                        # Cooldown: power_high to power_low (decreasing)
                        target_powers = power_high - (power_high - power_low) * progress
                    else:
                        # Ramp/Warmup: power_low to power_high (increasing)
    ...
    # Keep your existing statistical plots
    def plot_interval_compliance(self, output_dir: Path) -> None:
        """Plot compliance metrics by interval"""
        if not self.interval_stats:
            return
            
        stats_df = pd.DataFrame(self.interval_stats)
        fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(15, 6))
        
        # Power compliance
        if 'power_compliance_pct' in stats_df.columns:
            intervals = range(1, len(stats_df) + 1)
            ax1.bar(intervals, stats_df['power_compliance_pct'], color='skyblue', alpha=0.7)
            ax1.axhline(y=100, color='green', linestyle='--', label='Perfect Compliance')
            ax1.set_xlabel('Interval')
            ax1.set_ylabel('Power Compliance (%)')
            ax1.set_title('Power Compliance by Interval')
            ax1.legend()
            ax1.grid(True, alpha=0.3)
        
        # Cadence compliance
        if 'cadence_compliance_pct' in stats_df.columns:
            intervals = range(1, len(stats_df) + 1)
            ax2.bar(intervals, stats_df['cadence_compliance_pct'], color='lightcoral', alpha=0.7)
            ax2.axhline(y=100, color='green', linestyle='--', label='Perfect Compliance')
            ax2.set_xlabel('Interval')
            ax2.set_ylabel('Cadence Compliance (%)')
            ax2.set_title('Cadence Compliance by Interval')
            ax2.legend()
            ax2.grid(True, alpha=0.3)
        
        plt.tight_layout()
        plt.savefig(output_dir / 'interval_compliance.png', dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def plot_interval_performance(self, output_dir: Path) -> None:
        ...
        # Power variance
        if 'power_variance' in stats_df.columns:
            intervals = range(1, len(stats_df) + 1)
            colors = ['red' if x < 0 else 'green' for x in stats_df['power_variance']]
            ax1.bar(intervals, stats_df['power_variance'], color=colors, alpha=0.7)
            ax1.axhline(y=0, color='black', linestyle='-')
            ax1.set_xlabel('Interval')
            ax1.set_ylabel('Power Variance (W)')
            ax1.set_title('Power Variance (Actual - Target)')
            ax1.grid(True, alpha=0.3)
        
        # Heart rate drift
        if 'hr_drift_absolute' in stats_df.columns:
            intervals = range(1, len(stats_df) + 1)
            colors = ['red' if x > 0 else 'green' for x in stats_df['hr_drift_absolute']]
            ax2.bar(intervals, stats_df['hr_drift_absolute'], color=colors, alpha=0.7)
            ax2.axhline(y=0, color='black', linestyle='-')
            ax2.set_xlabel('Interval')
            ax2.set_ylabel('HR Drift (BPM)')
            ax2.set_title('Heart Rate Drift by Interval')
            ax2.grid(True, alpha=0.3)
        
        # Power standard deviation
        if 'power_std' in stats_df.columns:
            intervals = range(1, len(stats_df) + 1)
            ax3.bar(intervals, stats_df['power_std'], color='orange', alpha=0.7)
            ax3.set_xlabel('Interval')
            ax3.set_ylabel('Power Std Dev (W)')
            ax3.set_title('Power Consistency by Interval\n(Lower = More Consistent)')
            ax3.grid(True, alpha=0.3)
        
        # Cadence variance
        if 'cadence_variance' in stats_df.columns:
            intervals = range(1, len(stats_df) + 1)
            colors = ['red' if x < 0 else 'green' for x in stats_df['cadence_variance']]
            ax4.bar(intervals, stats_df['cadence_variance'], color=colors, alpha=0.7)
            ax4.axhline(y=0, color='black', linestyle='-')
            ax4.set_xlabel('Interval')
            ax4.set_ylabel('Cadence Variance (RPM)')
            ax4.set_title('Cadence Variance (Actual - Target)')
            ax4.grid(True, alpha=0.3)
        
        plt.tight_layout()
        plt.savefig(output_dir / 'interval_performance.png', dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def plot_compliance_heatmap(self, output_dir: Path) -> None:
        ...
        if 'power_compliance_pct' in stats_df.columns:
            heatmap_data.append(stats_df['power_compliance_pct'].values)
            metrics.append('Power Compliance')
        
        if 'cadence_compliance_pct' in stats_df.columns:
            heatmap_data.append(stats_df['cadence_compliance_pct'].values)
            metrics.append('Cadence Compliance')
        
        if heatmap_data:
            heatmap_array = np.array(heatmap_data)
            plt.figure(figsize=(10, 6))
            sns.heatmap(heatmap_array, annot=True, fmt='.1f', cmap='RdYlGn', 
                       xticklabels=[f'Int{i+1}' for i in range(len(stats_df))],
                       yticklabels=metrics, vmin=0, vmax=100)
            plt.title('Training Compliance Heatmap')
            plt.tight_layout()
            plt.savefig(output_dir / 'compliance_heatmap.png', dpi=300, bbox_inches='tight')
            plt.close()


def create_training_plots(enhanced_df: pd.DataFrame, statistics_results: Dict[str, Any], 
                         output_dir: Path, smoothing_params: Dict = None,
                         zwo_intervals: List[Dict] = None, metadata: Dict = None) -> None:
    """
    Convenience function to create all training plots.
    """
    plotter = TrainingPlotter(
        enhanced_df=enhanced_df,
        interval_stats=statistics_results.get('interval_statistics', []),
        overall_stats=statistics_results.get('overall_statistics', {}),
        smoothing_params=smoothing_params,
        zwo_intervals=zwo_intervals or [],
